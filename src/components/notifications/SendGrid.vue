<template>
    ...
    <div class="mb-3">
        <label for="sendgrid-from-email" class="form-label">{{
            $t("From Email")
        }}</label>
        <input
    ...
    <div class="mb-3">
        <label for="sendgrid-to-email" class="form-label">{{
            $t("To Email")
        }}</label>
        <input
    ...
    <div class="mb-3">
        <label for="sendgrid-cc-email" class="form-label">{{
            $t("smtpCC")
        }}</label>
        <input
    ...
    <div class="mb-3">
        <label for="sendgrid-bcc-email" class="form-label">{{
            $t("smtpBCC")
        }}</label>
        <input
    ...
    <div class="mb-3">
        <label for="sendgrid-subject" class="form-label">{{
            $t("Subject:")
        }}</label>
        <input
            id="sendgrid-subject"
            v-model="$parent.notification.sendgridSubject"
            type="text"
            class="form-control"
        />
        <small class="form-text text-muted">{{
    ...
    <i18n-t tag="p" keypath="More info on:" style="margin-top: 8px">
        <a
            ...
            target="_blank"
            >https://docs.sendgrid.com/api-reference/mail-send/mail-send</a
        >
    </i18n-t>
</template>
...
export default {
    ...
    mounted() {
        if (typeof this.$parent.notification.sendgridSubject === "undefined") {
            this.$parent.notification.sendgridSubject =
