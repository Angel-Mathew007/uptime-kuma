<template>
    <div class="mb-3">
        <label for="homeserver-url" class="form-label">{{
    ...
    <div class="mb-3">
        <label for="internal-room-id" class="form-label">{{
            $t("Internal Room Id")
        }}</label
        ><span style="color: red"><sup>*</sup></span>
        <input
    ...
    <div class="form-text">
        ...
        <i18n-t tag="p" keypath="matrixDesc2" style="margin-top: 8px">
            <code
                >curl -XPOST --json '{"type": "m.login.password", "identifier":
                {"user": "botusername", "type": "m.id.user"}, "password":
                "passwordforuser"}'
                "https://home.server/_matrix/client/v3/login"</code
            >.
        </i18n-t>
    </div>
</template>
