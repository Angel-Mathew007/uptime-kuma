...
test("Test ConditionExpressionGroup.fromMonitor", async (t) => {
    ...
    assert.strictEqual(
        true,
        root.children[1] instanceof ConditionExpressionGroup,
    );
    assert.strictEqual(true, root.children[1].children.length === 2);
    assert.strictEqual(
        true,
        root.children[1].children[0] instanceof ConditionExpression,
    );
    assert.strictEqual(true, root.children[1].children[0].value === "bar");
    assert.strictEqual(
        true,
        root.children[1].children[1] instanceof ConditionExpressionGroup,
    );
    assert.strictEqual(
        true,
        root.children[1].children[1].children.length === 1,
    );
    assert.strictEqual(
        true,
        root.children[1].children[1].children[0] instanceof ConditionExpression,
    );
    assert.strictEqual(
        true,
        root.children[1].children[1].children[0].value === "car",
    );
});
