...
class NextcloudTalk extends NotificationProvider {
    ...
    async send(notification, msg, monitorJSON = null, heartbeatJSON = null) {
        ...
        const talkRandom = encodeURIComponent(
            Crypto.randomBytes(64).toString("hex").slice(0, 64),
        );

        // Create the signature over random and message
        const talkSignature = Crypto.createHmac(
            "sha256",
            Buffer.from(notification.botSecret, "utf8"),
        )
            .update(Buffer.from(`${talkRandom}${msg}`, "utf8"))
            .digest("hex");

        let silentUp =
            heartbeatJSON?.status === UP && notification.sendSilentUp;
        let silentDown =
            heartbeatJSON?.status === DOWN && notification.sendSilentDown;
