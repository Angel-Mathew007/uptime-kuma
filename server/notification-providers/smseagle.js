        ...
        try {
            if (notification.smseagleApiType === "smseagle-apiv1") {
                ...
                if (notification.smseagleRecipientType === "smseagle-contact") {
                    ...
                    notification.smseagleRecipientType === "smseagle-group"
                ) {
                    recipientType = "groupname";
                    sendMethod = "/send_togroup";
                } else if (
                    notification.smseagleRecipientType === "smseagle-to"
                ) {
                    recipientType = "to";
                    sendMethod = "/send_sms";
                    if (notification.smseagleMsgType !== "smseagle-sms") {
                        ...
                        if (notification.smseagleMsgType === "smseagle-ring") {
                            ...
                            notification.smseagleMsgType === "smseagle-tts"
                        ) {
                            sendMethod = "/tts_call";
                        } else if (
                            notification.smseagleMsgType ===
                            "smseagle-tts-advanced"
                        ) {
                            sendMethod = "/tts_adv_call";
                            voiceId = notification.smseagleTtsModel
                                ? notification.smseagleTtsModel
                                : 1;
                        }
                    }
                }
                ...
                const url = new URL(
                    notification.smseagleUrl + "/http_api" + sendMethod,
                );

                url.searchParams.append(
                    "access_token",
                    notification.smseagleToken,
                );
                url.searchParams.append(
                    recipientType,
                    notification.smseagleRecipient,
                );
                if (
                    !notification.smseagleRecipientType ||
                    notification.smseagleRecipientType === "smseagle-sms"
                ) {
                    url.searchParams.append(
                        "unicode",
                        notification.smseagleEncoding ? "1" : "0",
                    );
                    url.searchParams.append(
                        "highpriority",
                        notification.smseaglePriority ?? "0",
                    );
                } else {
                ...
                let encoding = notification.smseagleEncoding
                    ? "unicode"
                    : "standard";
                ...
                if (notification.smseagleRecipientContact) {
                    postData["contacts"] = notification.smseagleRecipientContact
                        .split(",")
                        .map(Number);
                }
                if (notification.smseagleRecipientGroup) {
                    postData["groups"] = notification.smseagleRecipientGroup
                        .split(",")
                        .map(Number);
                }
                if (notification.smseagleRecipientTo) {
                    postData["to"] =
                        notification.smseagleRecipientTo.split(",");
                }
                ...
                if (notification.smseagleMsgType !== "smseagle-sms") {
                    ...
                    if (notification.smseagleMsgType === "smseagle-ring") {
                        ...
                        notification.smseagleMsgType === "smseagle-tts-advanced"
                    ) {
                        endpoint = "/calls/tts_advanced";
                        postData["voice_id"] =
                            notification.smseagleTtsModel ?? 1;
                    }
                }

                let resp = await axios.post(
                    notification.smseagleUrl + "/api/v2" + endpoint,
                    postData,
                    config,
                );

                const queuedCount = resp.data.filter(
                    (x) => x.status === "queued",
                ).length;
